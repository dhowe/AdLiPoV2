...
DOMList.prototype.css = function(attrs) {
    var i = this.nodes.length;
    if ( attrs === undefined ) {
        ...
        while ( i-- ) {
            for (var idx in attrs) {
                if ((idx === 'styles' || idx === 'style') && typeof attrs[idx] === 'object') {
                    for (var prop in attrs[idx]) { this.nodes[i].style[prop] = attrs[idx][prop]; }
                } else if (idx === 'html') {
                    this.nodes[i].innerHTML = attrs[idx];
                } else {
                    this.nodes[i].style.setProperty(idx, attrs[idx]);
                }
            }
